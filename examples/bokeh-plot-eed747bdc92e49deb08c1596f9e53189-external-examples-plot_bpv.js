(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("dff5112e-9a40-4ab6-991d-4d2a0872163a");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'dff5112e-9a40-4ab6-991d-4d2a0872163a' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js": "T2yuo9Oe71Cz/I4X9Ac5+gpEa5a8PpJCDlqKYO0CfAuEszu1JrXLl8YugMqYe3sM", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js": "98GDGJ0kOMCUMUePhksaQ/GYgB3+NH9h996V88sh3aOiUNX3N+fLXAtry6xctSZ6", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js": "89bArO+nlbP3sgakeHjCo1JYxYR5wufVgA3IbUvDY+K7w4zyxJqssu7wVnfeKCq8"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"53a17dcb-5fd1-475e-aa43-db424b4daa22":{"roots":{"references":[{"attributes":{},"id":"14568","type":"DataRange1d"},{"attributes":{"data":{},"selected":{"id":"14631"},"selection_policy":{"id":"14630"}},"id":"14612","type":"ColumnDataSource"},{"attributes":{"toolbars":[{"id":"14592"}],"tools":[{"id":"14582"},{"id":"14583"},{"id":"14584"},{"id":"14585"},{"id":"14586"},{"id":"14587"},{"id":"14588"},{"id":"14589"}]},"id":"14634","type":"ProxyToolbar"},{"attributes":{"below":[{"id":"14574"}],"center":[{"id":"14577"},{"id":"14581"},{"id":"14606"}],"left":[{"id":"14578"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"14604"},{"id":"14610"},{"id":"14615"}],"title":{"id":"14617"},"toolbar":{"id":"14592"},"toolbar_location":null,"x_range":{"id":"14566"},"x_scale":{"id":"14570"},"y_range":{"id":"14568"},"y_scale":{"id":"14572"}},"id":"14565","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"14630","type":"UnionRenderers"},{"attributes":{},"id":"14631","type":"Selection"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"14591","type":"PolyAnnotation"},{"attributes":{"source":{"id":"14612"}},"id":"14616","type":"CDSView"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14603","type":"Line"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"14629"},"selection_policy":{"id":"14628"}},"id":"14607","type":"ColumnDataSource"},{"attributes":{},"id":"14566","type":"DataRange1d"},{"attributes":{},"id":"14579","type":"BasicTicker"},{"attributes":{"axis":{"id":"14578"},"dimension":1,"ticker":null},"id":"14581","type":"Grid"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"14590","type":"BoxAnnotation"},{"attributes":{"toolbar":{"id":"14634"},"toolbar_location":"above"},"id":"14635","type":"ToolbarBox"},{"attributes":{},"id":"14583","type":"PanTool"},{"attributes":{"data_source":{"id":"14601"},"glyph":{"id":"14602"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14603"},"selection_glyph":null,"view":{"id":"14605"}},"id":"14604","type":"GlyphRenderer"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"14627"},"selection_policy":{"id":"14626"}},"id":"14601","type":"ColumnDataSource"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"14582"},{"id":"14583"},{"id":"14584"},{"id":"14585"},{"id":"14586"},{"id":"14587"},{"id":"14588"},{"id":"14589"}]},"id":"14592","type":"Toolbar"},{"attributes":{"overlay":{"id":"14591"}},"id":"14586","type":"LassoSelectTool"},{"attributes":{"bottom":1.4596776592684024,"fill_alpha":0.1,"fill_color":"#2a2eec","top":0.6850827603275129},"id":"14606","type":"BoxAnnotation"},{"attributes":{},"id":"14628","type":"UnionRenderers"},{"attributes":{},"id":"14585","type":"WheelZoomTool"},{"attributes":{},"id":"14588","type":"SaveTool"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"14608","type":"Line"},{"attributes":{},"id":"14629","type":"Selection"},{"attributes":{},"id":"14626","type":"UnionRenderers"},{"attributes":{},"id":"14621","type":"BasicTickFormatter"},{"attributes":{},"id":"14623","type":"BasicTickFormatter"},{"attributes":{},"id":"14582","type":"ResetTool"},{"attributes":{"callback":null},"id":"14589","type":"HoverTool"},{"attributes":{"source":{"id":"14601"}},"id":"14605","type":"CDSView"},{"attributes":{"formatter":{"id":"14621"},"ticker":{"id":"14575"}},"id":"14574","type":"LinearAxis"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14602","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14614","type":"Line"},{"attributes":{},"id":"14627","type":"Selection"},{"attributes":{},"id":"14570","type":"LinearScale"},{"attributes":{"overlay":{"id":"14590"}},"id":"14584","type":"BoxZoomTool"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"14617","type":"Title"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"14609","type":"Line"},{"attributes":{"source":{"id":"14607"}},"id":"14611","type":"CDSView"},{"attributes":{"formatter":{"id":"14623"},"ticker":{"id":"14579"}},"id":"14578","type":"LinearAxis"},{"attributes":{},"id":"14572","type":"LinearScale"},{"attributes":{"children":[[{"id":"14565"},0,0]]},"id":"14633","type":"GridBox"},{"attributes":{"children":[{"id":"14635"},{"id":"14633"}]},"id":"14636","type":"Column"},{"attributes":{"data_source":{"id":"14612"},"glyph":{"id":"14613"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14614"},"selection_glyph":null,"view":{"id":"14616"}},"id":"14615","type":"GlyphRenderer"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14613","type":"Line"},{"attributes":{},"id":"14587","type":"UndoTool"},{"attributes":{"axis":{"id":"14574"},"ticker":null},"id":"14577","type":"Grid"},{"attributes":{},"id":"14575","type":"BasicTicker"},{"attributes":{"data_source":{"id":"14607"},"glyph":{"id":"14608"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14609"},"selection_glyph":null,"view":{"id":"14611"}},"id":"14610","type":"GlyphRenderer"}],"root_ids":["14636"]},"title":"Bokeh Application","version":"2.2.3"}}';
                  var render_items = [{"docid":"53a17dcb-5fd1-475e-aa43-db424b4daa22","root_ids":["14636"],"roots":{"14636":"dff5112e-9a40-4ab6-991d-4d2a0872163a"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();